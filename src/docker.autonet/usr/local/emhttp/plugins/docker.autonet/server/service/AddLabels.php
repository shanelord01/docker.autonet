...
const QUOTE_REPLACER = "\`";

$data = json_decode($_POST["data"]);

$containerNames = $data->containers;
$inputs = array_map(function ($item) {
    $item->name = str_replace(QUOTE_REPLACER, '"', $item->name);
    $item->value = str_replace(QUOTE_REPLACER, '"', $item->value);
    $item->key = str_replace(QUOTE_REPLACER, '"', $item->key);
    return $item;
}, $data->labels);
